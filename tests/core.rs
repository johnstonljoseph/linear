use linear::{
    BuiltinOp, CollectionMutability, Component, CoreError, CoreProgram, DeclaredCapabilities,
    Evaluator, Expr, Function, GlobalDef, GlobalExpr, MatchArm, Param, Statement, TypeStore, Value,
    ValueId,
};
...
#[test]
fn builtin_list_build_len_and_get_check_and_run() {
    let mut types = TypeStore::new();
    let u32_ty = u32_type(&mut types);
    let list_ty = types
        .add_list(
    ...
    let function = program
        .add_function(Function {
            name: Some("list_program".into()),
            inputs: vec![],
            outputs: vec![list_ty, u32_ty, u32_ty],
            body: vec![
                Statement::new(
                    vec![ValueId(0)],
                    Expr::Builtin {
                ...
                Statement::new(
                    vec![ValueId(5), ValueId(6)],
                    Expr::Builtin {
                        op: BuiltinOp::ListLen {
                            list_ty,
                            size_ty: u32_ty,
                        },
                        args: vec![ValueId(4)],
                ...
                Statement::new(
                    vec![ValueId(8), ValueId(9)],
                    Expr::Builtin {
                        op: BuiltinOp::ListGet {
                            list_ty,
                            index_ty: u32_ty,
                        },
                        args: vec![ValueId(5), ValueId(7)],
                    },
                ),
            ],
            returns: vec![ValueId(8), ValueId(6), ValueId(9)],
        })
        .unwrap();
...
#[test]
fn immutable_lists_can_be_duplicated_when_elements_can_be_duplicated() {
    let mut types = TypeStore::new();
    let u32_ty = u32_type(&mut types);
    let list_ty = types
        .add_vector(
            Some("ImmutableVectorU32".into()),
            u32_ty,
    ...
    let function = program
        .add_function(Function {
            name: Some("immutable_list_dup".into()),
            inputs: vec![],
            outputs: vec![list_ty, list_ty, u32_ty],
            body: vec![
                Statement::new(
                    vec![ValueId(0)],
                    Expr::Builtin {
            ...
            returns: vec![ValueId(4), ValueId(5), ValueId(6)],
        })
        .unwrap();
...
#[test]
fn builtin_hashmap_check_and_run() {
    let mut types = TypeStore::new();
    let u32_ty = u32_type(&mut types);
    let bool_ty = bool_type(&mut types);
    let map_ty = types
        .add_hashmap(
            Some("HashMapU32U32".into()),
            u32_ty,
            u32_ty,
            CollectionMutability::Mutable,
        )
    ...
    let function = program
        .add_function(Function {
            name: Some("hashmap_program".into()),
            inputs: vec![],
            outputs: vec![map_ty, u32_ty, bool_ty],
            body: vec![
                Statement::new(
                    vec![ValueId(0)],
                    Expr::Builtin {
                ...
                Statement::new(
                    ...
                    Expr::Builtin {
                        ...
                        args: vec![ValueId(3), ValueId(4), ValueId(5)],
                    },
                ),
                Statement::new(
                    vec![ValueId(8)],
                ...
                Statement::new(
                    ...
                    Expr::Builtin {
                        op: BuiltinOp::HashMapContains { map_ty, bool_ty },
                        args: vec![ValueId(6), ValueId(8)],
                    },
                ),
            ],
            returns: vec![ValueId(9), ValueId(7), ValueId(10)],
        })
        .unwrap();

    assert_eq!(program.check(&types), Ok(()));
    let output = Evaluator::new(&types, &program)
        .run_function(function, vec![])
        .unwrap();
    assert_eq!(output[1], Value::Finite(42));
    assert_eq!(
        output[2],
        Value::Sum {
            variant: 1,
